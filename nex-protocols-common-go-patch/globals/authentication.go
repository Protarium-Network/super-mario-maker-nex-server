...
func ValidatePretendoLoginData(pid types.PID, loginData types.DataHolder, aesKey []byte) *nex.Error {
	...
	Logger.Infof("[LOGIN-DEBUG] loginDataType=%q raw object=%#v", loginDataType, loginData.Object)
	...
	case "NintendoLoginData": // * Wii U
		...
		Logger.Infof("[LOGIN-DEBUG] NintendoLoginData.Token=%q (len=%d)", tokenBase64, len(tokenBase64))
	case "AccountExtraInfo": // * 3DS
		...
		Logger.Infof("[LOGIN-DEBUG] AccountExtraInfo.NEXToken=%q (len=%d)", tokenBase64, len(tokenBase64))
	...
	case "AuthenticationInfo": // * 3DS / Wii U
		...
		Logger.Infof("[LOGIN-DEBUG] AuthenticationInfo.Token=%q (len=%d) NGSVersion=%v TokenType=%v", tokenBase64, len(tokenBase64), authenticationInfo.NGSVersion, authenticationInfo.TokenType)
		tokenBase64 = strings.Replace(tokenBase64, ".", "+", -1)
		tokenBase64 = strings.Replace(tokenBase64, "-", "/", -1)
		tokenBase64 = strings.Replace(tokenBase64, "*", "=", -1)
	default:
