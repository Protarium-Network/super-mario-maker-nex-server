...
func CanJoinMatchmakeSession(manager *MatchmakingManager, pid types.PID, matchmakeSession match_making_types.MatchmakeSession) *nex.Error {
	...
	if matchmakeSession.ParticipationPolicy == 98 {
		// Bypassed for private server: Anyone can join friends-only lobbies
	}
...
func SendNotificationEvent(endpoint *nex.PRUDPEndPoint, event notifications_types.NotificationEvent, targets []uint64) {
	...
	for _, pid := range targets {
		...
		if target == nil {
			fmt.Printf("[NOTIFICATION] ÉCHEC cible PID=%d non connectée (type=0x%X, param1=%d, param2=%d)\n",
				pid, uint32(event.Type), uint32(event.Param1), uint32(event.Param2))
			continue
		}
		...
		fmt.Printf("[NOTIFICATION] Envoi cible PID=%d CID=%d stream=%d PRUDP=v%d type=0x%X param1=%d param2=%d param3=%d callID=%d bytes=%d\n",
			pid, target.ID, target.StreamID, target.DefaultPRUDPVersion,
			uint32(event.Type), uint32(event.Param1), uint32(event.Param2), uint32(event.Param3),
			rmcRequest.CallID, len(rmcRequestBytes))
