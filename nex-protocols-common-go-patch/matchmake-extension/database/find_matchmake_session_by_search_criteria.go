...
func FindMatchmakeSessionBySearchCriteria(manager *common_globals.MatchmakingManager, connection *nex.PRUDPConnection, searchCriterias []match_making_types.MatchmakeSessionSearchCriteria, resultRange types.ResultRange, sourceMatchmakeSession *match_making_types.MatchmakeSession) ([]match_making_types.MatchmakeSession, *nex.Error) {
	...
	for _, searchCriteria := range searchCriterias {
		searchStatement := `SELECT
			...
			(CASE WHEN g.participation_policy=98 THEN true ELSE true END) AND
			(CASE WHEN $4=true THEN ms.open_participation=true ELSE true END) AND
			(CASE WHEN $5=true THEN g.host_pid <> 0 ELSE true END) AND
			(CASE WHEN $6=true THEN ms.user_password_enabled=false ELSE true END) AND
			(CASE WHEN $7=true THEN ms.system_password_enabled=false ELSE true END)`
		...
		case constants.SelectionMethodNearestNeighbor:
			...
			if err != nil {
				common_globals.Logger.Warning("NearestNeighbor: Attribs[1] parsing failed, defaulting to 0")
				attribute1 = 0
			}
		...
		case constants.SelectionMethodBroadenRange:
			...
			attribute1, err := strconv.ParseUint(string(searchCriteria.Attribs[1]), 10, 32)
			if err != nil {
				common_globals.Logger.Warning("BroadenRange: Attribs[1] parsing failed, defaulting to 0")
				attribute1 = 0
			}
		...
		case constants.SelectionMethodBroadenRangeWithProgressScore:
			...
			if searchCriteria.Attribs[1] != "" {
				attribute1, err := strconv.ParseUint(string(searchCriteria.Attribs[1]), 10, 32)
				if err != nil {
					common_globals.Logger.Warning("Block 3: Attribs[1] parsing failed, defaulting to 0")
					attribute1 = 0
				}
