...
const wiiSportsClubGameModeRegionMask uint32 = 0xC00

func canonicalWiiSportsClubGameMode(gameMode types.UInt32) types.UInt32 {
	return types.NewUInt32(uint32(gameMode) &^ wiiSportsClubGameModeRegionMask)
}
...
func (commonProtocol *CommonProtocol) autoMatchmakeWithSearchCriteriaPostpone(err error, packet nex.PacketInterface, callID uint32, lstSearchCriteria types.List[match_making_types.MatchmakeSessionSearchCriteria], anyGathering match_making_types.GatheringHolder, strMessage types.String) (*nex.RMCMessage, *nex.Error) {
	...
	// Keep the console's regional value for its response, but use one canonical
	// value for database search/storage. Returning the canonical value to a
	// console from another region leaves Wii Sports Club's client state machine
	// waiting even after the PIA mesh has connected.
	requestedGameMode := matchmakeSession.GameMode
	matchmakeSession.GameMode = canonicalWiiSportsClubGameMode(requestedGameMode)
	...
	responseSession := resultSession.Copy().(match_making_types.MatchmakeSession)
	responseSession.GameMode = requestedGameMode

	matchmakeDataHolder := match_making_types.NewGatheringHolder()
	matchmakeDataHolder.Object = responseSession
