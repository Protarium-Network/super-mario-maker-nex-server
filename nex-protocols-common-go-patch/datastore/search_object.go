...
func (commonProtocol *CommonProtocol) searchObject(err error, packet nex.PacketInterface, callID uint32, param datastore_types.DataStoreSearchParam) (*nex.RMCMessage, *nex.Error) {
	...
	sentIDs := make([]uint64, 0, len(objects))

	for _, object := range objects {
		errCode = commonProtocol.VerifyObjectPermission(object.OwnerID, connection.PID(), object.Permission)
		if errCode != nil {
			fmt.Printf("[SearchObject response] pid=%d filtered data_id=%d owner=%d permission=%d\n", connection.PID(), object.DataID, object.OwnerID, object.Permission.Permission)
		...
		sentIDs = append(sentIDs, uint64(object.DataID))
		fmt.Printf("[SearchObject response] pid=%d data_id=%d data_type=%d size=%d owner=%d permission=%d tags=%v\n", connection.PID(), object.DataID, object.DataType, object.Size, object.OwnerID, object.Permission.Permission, object.Tags)
	}
	fmt.Printf("[SearchObject response] pid=%d query_data_type=%d owner_type=%d query_tags=%v db_results=%d sent_results=%d sent_ids=%v\n", connection.PID(), param.DataType, param.OwnerType, param.Tags, len(objects), len(pSearchResult.Result), sentIDs)
